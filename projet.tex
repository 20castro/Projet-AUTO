...
\]
\[
	u = - \lambda ( \theta_m - \theta_r ) \left[ - \mu \dot \theta_m \text{ pour l'instant non, à ajouter pour PD} \right]
\]
...
on considère $\lambda$ grand donc $\varepsilon = \frac{1}{\lambda}$ petit. La dernière équation devient :
...
\noindent Par perturbations singulières, on peut simplifier $\theta = - k_i \eta - k x - k_d v$.

\noindent ... ue-stable

\noindent On considère alors la nouvelle variable d'état : $(x, v, \eta)$. [ Si PD $\to$ $(x, v, a, \eta)$]
...
\noindent On néglige toujours les deux bruits de mesure $\nu_x$ et $\nu_\theta$. Ce contrôleur donne alors le linéarisé :
...
On note $\fact{} = \gamma$ dans la suite. Alors on obtient le polynôme caractéristique : ...
